...
if (isset($_GET['action'], $_GET['consultation_id']) && $_GET['action'] === 'cancel') {
    $cid = (int)$_GET['consultation_id'];
    $stmt = $conn->prepare("
    ...
    header("Location: consultations.php#pending");
    exit();
}
...
// Fetch symptom list for the “related symptom” dropdown
$stmt = $conn->prepare("
  SELECT id, symptom_name, symptom_date 
    FROM symptoms 
   WHERE user_id = ? 
              ...
              <div class="consultation-content">
                ...
                <div class="consultation-details">
                  ...
                  <div class="detail-item">
                    🔍 Reason: <?= htmlspecialchars("{$c['symptom_name']}, " . ucfirst($c['symptom_severity'])) ?>
                  </div>
                </div>
          ...
          <?php else: foreach($pending as $c): ?>
            <div class="consultation-card">
              ...
              <div class="consultation-content">
                <div class="consultation-details">
                  <div class="detail-item">
                    📝 Requested: <?= date('F j, Y', strtotime($c['consultation_date'])) ?>
                  </div>
                  <div class="detail-item">
                    👨‍⚕️ Doctor Type: <?= htmlspecialchars(ucfirst($c['specialization'])) ?>
                  </div>
                  <div class="detail-item">
                    🔍 Reason: <?= htmlspecialchars("{$c['symptom_name']}, " . ucfirst($c['symptom_severity'])) ?>
                  </div>
                  <div class="detail-item">
                    🏥 Preferred Type: <?= ucfirst($c['consultation_type']) ?>
                  </div>
                </div>
                ...
                <div class="consultation-actions">
                  <!-- now a link that actually cancels -->
                  <a href="consultations.php?action=cancel&consultation_id=<?= $c['id'] ?>"
        ...
        <div class="table-container">
          ...
          <?php else: ?>
            <table class="data-table">
              <thead>
                <tr>
                  <th>Date</th><th>Doctor</th><th>Specialty</th>
                  <th>Reason</th><th>Type</th><th>Diagnosis</th><th>Actions</th>
                </tr>
              </thead>
              <tbody>
